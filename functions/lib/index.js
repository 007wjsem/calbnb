...
exports.provisionCompany = void 0;
 ...
 */
exports.provisionCompany = functions.https.onCall(async (data, context) => {
    ...
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'You must be logged in to create a company.');
    }
    ...
    if (!callerData || callerData.role !== 'Administrator' || callerData.companyId != null) {
        throw new functions.https.HttpsError('permission-denied', 'Only Super Administrators can provision companies.');
    }
    const { companyName, ownerEmail, tier } = data;
    if (!companyName || !ownerEmail || !tier) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields (companyName, ownerEmail, tier).');
    }
    let ownerUid;
    try {
